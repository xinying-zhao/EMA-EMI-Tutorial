...
面对生态瞬时评估（EMA）产生的大规模、密集型追踪数据，跨学科（健康科学、心理学、社会科学）的新手研究者常因缺乏合适的统计学范式而陷入误区。EMA 数据本质上是一种典型的长格式（Long-format）数据集。一个常见的错误做法是将同一个体在两周内收集的数十次测量值做简单的均值聚合，随后使用传统的 OLS 回归或方差分析（ANOVA）做检验。这种做法不仅抹杀了 EMA 研究最为珍贵的时间动态演变信息，更严重违背了经典统计学中关于"残差独立性"的基本假设。由于同一个体在相近时间点上的行为、情绪或生理状态存在高度的自相关性，简单聚合会导致标准误的严重低估，进而引发第一类错误（假阳性）的激增。
...
**群体相关不等于个体相关**。即使不聚合数据、直接把所有测量点当作独立样本做相关或回归，也同样会掉进“生态学谬误”（Ecological fallacy）。经典的例子来自打字行为研究（Hamaker, 2012）：在**群体层面**，打字更快的人往往拼写错误更少（因为熟练者的整体技术更好）；但在**个体层面**，同一个人打字越快、犯的拼写错误反而越多。两个层面的相关方向甚至可能相反。EMA 数据是同一个体的重复测量，分析时必须明确区分两类关系：**个体间（Between-person）**关系回答”压力更大的人是否更不快乐”，**个体内（Within-person）**关系回答”一个人压力升高的那个时刻，是否也更不快乐”。混淆两者，是 EMA 分析中最常见也最具破坏性的错误。

![群体相关不等于个体相关：打字速度与错误率](images/fig_ch06_between_within_typing.png)

*图 6-1：群体层面（左）与个体层面（右）的关系可能完全不同。左图：横断面（个体间）显示打字越快错误率越低；右图：对每个人而言，打字越快的时刻错误率反而越高。引自 Hamaker（2012，Figure 3.1），载于 Mehl & Conner 主编的《Handbook of Research Methods for Studying Daily Life》，Guilford Press。*
...
- **步骤二：执行组内中心化（CWC）**。针对 Level 1 的瞬时变量，执行组内中心化（Centering Within Clusters），即用受试者每次的报告值减去该受试者在整个研究期间的均值。这一操作能够精准隔离出个体内纯粹的状态波动，用于回答"一个人此刻是否比其平时的平均状态更加焦虑（或更有运动意愿）"等问题。
...
- **步骤四：构建与比较多层线性模型**。使用 R 语言中的 `lme4` 包，首先建立无预测变量的零模型（Null Model）计算组内相关系数（ICC），评估使用 MLM 的必要性。ICC 衡量总变异中由个体间差异解释的比例，取值介于 0–1；ICC 越大（经验上 > 0.10），表明数据嵌套性越强，越有必要采用多层模型。随后建议采用“三步递进”（3-fold）策略逐级增加模型复杂度，以判断哪些随机结构是数据真正需要的（Nezlek, 2012）：
  1. **群体层面朴素回归（Naïve regression）**：先忽略嵌套结构，用全部观测拟合普通回归（R：`lm(happy ~ stress)`），作为比较基准；
  2. **随机截距模型（Random intercept）**：允许每个个体拥有各自的基线水平，截距围绕总体均值呈正态分布（R：`lmer(happy ~ stress + (1|id))`）；
  3. **随机斜率模型（Random slope）**：在随机截距的基础上，进一步允许“变量间关系”的斜率因人而异（R：`lmer(happy ~ stress + (1 + stress|id))`）。
  相邻模型是嵌套的（Nested），可用**似然比检验（Likelihood-ratio test）**比较拟合优度（R：`anova(model2, model3)`）：若随机截距模型显著优于朴素回归，说明个体基线差异不可忽略；若随机斜率模型显著优于随机截距模型，说明个体对研究变量（如压力）的敏感性存在真实异质性。固定效应、随机效应结构、误差协方差结构与模型拟合评估均应在论文中完整报告（Luna et al., 2025）。
...
*图 6-2：EMA 动态特征的三类量化维度：不稳定性（RMSSD）、变异性（方差）与时序依赖（自相关）。引自 Schoevers et al.（2021），*Psychological Medicine*，[DOI: 10.1017/S0033291720000689](https://doi.org/10.1017/S0033291720000689)，CC BY 4.0 许可。*
...
选择何种动态指标应**由理论假设驱动**而非数据驱动：若研究关注情绪的恢复能力，优先选择 AR(1)；若关注波动的剧烈程度，优先选择 MSSD。同时，移动认知任务数据（如反应时、准确率）的个体内变异性（IIV）本身即可作为认知稳定性的指标进行分析（Luna et al., 2025）。

## 6.4 个体内（n=1）分析：自上而下之外的路径

多水平模型从群体出发、同时刻画个体差异，是 EMA 数据分析的“自上而下”（Top-down）路径；而**个体内分析**（Within-person analysis，即 n=1 分析）则只针对单个个体，把某一个人的全部测量当作独立数据集，探索“什么因素让这个人（而非人群）更快乐或更焦虑”。这一思路在量化自我（Quantified Self）研究与临床个案纵向追踪中尤为常见，其价值在于：

- **回答”我”的问题**：”我自己在压力升高的时刻是否更不快乐”，而非”压力大的人是否更不快乐”；
- **自下而上的探索**：先深入理解个体，再在群体中检验相似性或差异性（Hamaker, 2012）；
- **生成新假设**：个体自身的生活经验可作为解读数据的补充信息，甚至发现群体模型中被“平均掉”的独特模式。

具体方法的选择取决于变量类型：**连续 & 连续**用回归（如“忙碌程度”预测“投入度”）；**连续 & 二分**用 t 检验（如“是否喜欢当前活动”两组的投入度差异）；**二分 & 二分**用卡方检验（如“喜欢当前活动”与“喜欢在场的人”之间的关联）。

需要警惕的是，n=1 分析与多水平模型的结论可能不同甚至相反：群体模型把个体斜率“压缩”向均值，可能低估某个体更极端的真实关系；反之，个体分析的结论也未必能推广到群体。因此两者应视为互补而非替代：多水平模型回答群体层面的规律，n=1 分析提供个体层面的深度。
...
- Nezlek, J. B. (2012). Multilevel modeling analyses of diary-style data. In M. R. Mehl & T. S. Conner (Eds.), *Handbook of Research Methods for Studying Daily Life* (pp. 357–383). Guilford Press.
